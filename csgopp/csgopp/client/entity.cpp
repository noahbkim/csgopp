...
/// \todo Perhaps this should be in client.h where the rest of the parsing is
...
void Entity::update(BitStream& stream)
{
    ...
    for (uint16_t i : indices)
    {
        ...
        const entity::Offset& offset = this->type->prioritized.at(i);
        offset.type->update(this->address + offset.offset, stream, offset.property);
    }
}
